...
const BlogPage = () => {
  ...
  const filteredPosts = activeCategory ? posts.filter(post => post.categories.includes(activeCategory)) : posts;
  ...
  return <Layout>
      <div className="container mx-auto px-4 py-12">
        {/* Hero Section */}
        <div className="relative mb-10 rounded-xl overflow-hidden shadow-lg" style={{
            ...
            <h1 className="text-3xl md:text-5xl font-bold mb-4 leading-tight">
              ...
              <div className="inline md:inline-block md:ml-3">
                <span className="block font-bold md:inline">
                  <span className="tracking-tight font-extrabold">Osome</span>&nbsp;
        ...
        <div className="mb-8 flex flex-wrap gap-2 justify-center">
          <Button variant={!activeCategory ? "default" : "outline"} className="mb-2 shadow-sm transition-all hover:shadow" onClick={() => handleCategoryClick("")}>
            All
          </Button>
          {allCategories.map(category => <Button key={category} variant={activeCategory === category ? "default" : "outline"} className="mb-2 shadow-sm transition-all hover:shadow" onClick={() => handleCategoryClick(category)}>
              {category}
            </Button>)}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          {currentPosts.map(post => <Card key={post.slug} className="flex flex-col h-full overflow-hidden shadow-lg border border-gray-200 rounded-xl bg-[#EBF1FE]">
              ...
              <CardFooter className="pt-0 pb-4">
                <Button variant="outline" className="w-fit px-5 left-0 flex items-center gap-2 border-black border-2
                    hover:bg-black hover:text-white hover:border-black transition
                    font-semibold capitalize" onClick={() => handleReadMore(post.slug)}>
                  Read More
        ...
        {currentPosts.length === 0 && <div className="text-center py-12 border rounded-lg bg-gray-50">
            <p className="text-lg text-gray-600">No posts found for this category</p>
            <Button variant="outline" className="mt-4" onClick={() => handleCategoryClick("")}>
              View All Posts
            </Button>
          </div>}
        ...
        {totalPages > 1 && <Pagination className="mt-8">
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious onClick={() => handlePageChange(currentPage - 1)} className={currentPage === 1 ? "pointer-events-none opacity-50" : "cursor-pointer hover:bg-gray-100"} />
              </PaginationItem>
              
              {Array.from({
            length: totalPages
          }).map((_, index) => <PaginationItem key={index}>
                  <PaginationLink isActive={currentPage === index + 1} onClick={() => handlePageChange(index + 1)} className="hover:bg-gray-100">
                    {index + 1}
                  </PaginationLink>
                </PaginationItem>)}
              
              <PaginationItem>
                <PaginationNext onClick={() => handlePageChange(currentPage + 1)} className={currentPage === totalPages ? "pointer-events-none opacity-50" : "cursor-pointer hover:bg-gray-100"} />
              </PaginationItem>
            </PaginationContent>
          </Pagination>}
      </div>
