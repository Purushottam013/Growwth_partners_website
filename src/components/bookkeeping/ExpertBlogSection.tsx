  ...
  return <section className="bg-gradient-to-br from-gray-50 to-white py-[60px]">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {blogs.map((blog, index) => <motion.div key={index} initial={{
          opacity: 0,
          y: 20
        }} whileInView={{
          opacity: 1,
              ...
              <a href={blog.url} target="_blank" rel="noopener noreferrer" className="block h-full">
                <Card className="bg-white hover:shadow-xl transition-all duration-300 h-full hover:-translate-y-1 border border-gray-100">
                  <CardContent className="p-6">
                    <h4 className="text-xl font-bold mb-3 text-brand-dark group-hover:text-brand-green transition-colors">{blog.title}</h4>
